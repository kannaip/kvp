...
from royalties_extracts.transformations.prematch_other_US import process, writeDf


def run(start_day, end_day, env) -> None:
    spark = SparkSession.builder.appName("PreMatchOtherUSApp").getOrCreate()
    dbutils = DBUtils(spark)
    atlas_df, combined_df, inmarket_df = process(spark, start_day, end_day,
                        f"""curated_{env}.royalties.item_royalties_daily""",
                        f"""foreign_redshift_edw_{env}.edw_ods.playlist""",
                        f"""foreign_redshift_edw_{env}.edw_datamart.v_dim_channel""",
